...
def formula_graph_collate(batch):
    ...
    pyg_batch = Batch.from_data_list([Data(
        ...
        y=data.y,   
        x_s_batch=torch.full((data.x_s.size(0), i) if data.x_s.numel() > 0 else torch.empty(0)),
        x_t_batch=torch.full((data.x_t.size(0), i) if data.x_t.numel() > 0 else torch.empty(0))
